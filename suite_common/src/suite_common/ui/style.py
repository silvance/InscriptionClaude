"""Application-wide visual styling for every suite app.

The three suite apps (Inscription / CaseForge / CaseGuide) share a
single visual language: Segoe UI typography, a subdued neutral
palette, accent blue for primary actions, red reserved for
destructive / recording state. Light and dark variants are picked
from the system colour scheme on application start.

Apply once via :func:`apply_global_style` from the app bootstrap.
Individual widgets shouldn't set their own ``setStyleSheet`` for
things this file already covers — global control is what makes the
suite feel coherent rather than slapped together.

This file used to be duplicated three ways (one near-identical copy
per app). The bulk now lives here; each app's ``ui/style.py`` is a
one-line re-export so the import path operators may have memorised
(``from <app>.ui.style import apply_global_style``) keeps working.
"""
