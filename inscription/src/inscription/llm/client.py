...
:class:`StepRewriter` needs. Any OpenAI-compatible endpoint works:
Ollama's ``/v1``, LM Studio's server, ``llama.cpp --server``, or a
remote provider.

The client exposes one method, :meth:`chat`. Failures raise
:class:`LLMError` (or a subclass) with a human-readable message; the
caller is expected to show that to the user and fall back to the
